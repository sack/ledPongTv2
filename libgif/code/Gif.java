...
public class Gif extends PImage implements PConstants, Runnable {
	 ...
	 */
	public void run() {
		while (Thread.currentThread() == runner && !disposed) {
			try {
				Thread.sleep(5);
	 ...
	 * returns the delay
	 */
	public static int getDelay(PApplet parent, String filename) {
		GifDecoder gifDecoder = createDecoder(parent, filename);
		return gifDecoder.getDelay(1);
	}
